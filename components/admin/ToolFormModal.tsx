...
export function ToolFormModal({ tool, isOpen, onClose, defaultValues }: ToolFormModalProps) {
  ...
  const [form, setForm] = useState({
    ...
    display_order: defaultValues?.display_order ?? tool?.display_order ?? 0,
  })

  // Reset form when tool or defaultValues changes (fixes stale useState on re-open)
  useEffect(() => {
    if (!isOpen) return
    setForm({
      name: defaultValues?.name ?? tool?.name ?? '',
      ...
      <div className="bg-white dark:bg-gray-800 rounded-t-2xl sm:rounded-2xl border-0 sm:border border-gray-100 dark:border-gray-700 shadow-2xl w-full sm:max-w-xl max-h-[92vh] sm:max-h-[90vh] overflow-y-auto p-5 sm:p-6 animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1" title="Lower = appears first for students. Use 10, 20, 30… to leave room for reordering.">
                Display Order
              </label>
              <input
                type="number"
          ...
          <div className="flex gap-2 pt-3">
            ...
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-700 text-gray-600 text-sm font-medium hover:bg-gray-50 transition"
            >
              Cancel
            </button>
          </div>

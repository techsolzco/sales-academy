...
export function QuizTaker({ quiz }: Props) {
  ...
  const [result, setResult] = useState<{
    ...
    passed: boolean; correctCount: number; totalCount: number
  } | null>(null)
  ...
  const handleSubmit = async () => {
    ...
    setSubmitError(null)
    const answersArray = Object.entries(answers).map(([questionId, selectedOptionId]) => ({ questionId, selectedOptionId }))
    try {
    ...
    <div className="max-w-3xl mx-auto py-8 px-4">
      ...
      <div className="space-y-8 mb-8">
        {quiz.questions?.map((q: any, index: number) => (
          <div key={q.id} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6 md:p-8">
            <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100 mb-1">
              <span className="text-brand-600 dark:text-brand-400 mr-2">{index + 1}.</span>
              {q.question_text}
            </h3>
            <p className="text-sm text-gray-400 dark:text-gray-500 mb-6">{q.points || 1} point{(q.points || 1) !== 1 ? 's' : ''}</p>
                  ...
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-xl border-2 transition-all ${
                      isSelected
                        ...
                        : 'border-gray-100 dark:border-gray-700 hover:border-brand-200 dark:hover:border-brand-600 hover:bg-gray-50 dark:hover:bg-gray-700/50'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
                        ...
                        {isSelected && <div className="w-2.5 h-2.5 rounded-full bg-brand-600 dark:bg-brand-400" />}
                      </div>
                      <span className={`font-medium ${isSelected ? 'text-brand-900 dark:text-brand-200' : 'text-gray-700 dark:text-gray-300'}`}>
                        {opt.option_text}
                      </span>
                    </div>
                  </button>
      ...
      <div className="mb-4 flex items-center justify-between text-sm text-gray-500 dark:text-gray-400">
        ...
        <span className="text-xs text-gray-400 dark:text-gray-500">All questions required</span>
      </div>

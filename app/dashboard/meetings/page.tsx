...
export default async function DashboardMeetingsPage() {
  ...
  const renderMeetingCard = (meeting: any) => {
    ...
    return (
      <div key={meeting.id} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6 flex flex-col relative overflow-hidden">
        {isLive && (
          <div className="absolute top-0 left-0 w-1 h-full bg-red-500"></div>
        )}
        <div className="flex justify-between items-start mb-4">
          <span className={`px-2.5 py-1 rounded-full text-xs font-semibold uppercase tracking-wider ${
            isLive ? 'bg-red-100 text-red-700 animate-pulse' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300'
          }`}>
        ...
        {meeting.description && (
          <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 mb-4 line-clamp-2">{meeting.description}</p>
        )}
        
        <div className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-6 flex items-center gap-2 mt-auto pt-4">
          <Calendar className="w-4 h-4 text-brand-600" />
          {new Date(meeting.scheduled_at).toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' })}
        </div>

        <div className="grid grid-cols-2 gap-3 mt-auto border-t border-gray-100 dark:border-gray-700 pt-4">
          <a
            href={meeting.jitsi_url}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center justify-center gap-2 w-full py-2 rounded-xl bg-gray-50 dark:bg-gray-900 text-gray-700 dark:text-gray-300 font-medium hover:bg-gray-100 dark:bg-gray-700 transition-colors text-sm"
          >
  ...
  return (
    <div className="px-4 py-5 md:p-8 max-w-5xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-gray-500 dark:text-gray-400 dark:text-gray-500 mt-1">Join your scheduled sales training and syncs.</p>
      </div>
